...
;(function($){
	...
	function search_initialize(){
		...
		if(mapSearchBox.length && mapBox.length){
			...
			var clusterStyles = [{
					...
					url:'/assets/images/icon/cloud.png',
					height: 72,
					width: 72,
					textSize:15
				}
	...
	function mylocationCallback(pos){
		  var shape = {
		   ...
		   var marker = new google.maps.Marker({
	             ...
	             icon: '/assets/images/icon/my-marker.png',
	             shape: shape,
	             zIndex: 9999,
	...
	function single_initialize(){
		...
		console.log($('.property-map-box').data('address'));
		geocoder.geocode({'address':$('.property-map-box').data('address')},function(results, status){
			if (status == google.maps.GeocoderStatus.OK) {
				
				$('.property-map-box').attr("data-latitude", results[0].geometry.location.A);
				$('.property-map-box').attr("data-longitude", results[0].geometry.location.F);
